...
/**
 * A class to manage the unit tests of the {@link b2JointId} class.
 *
 * @author wil
 * @version 1.0.0
 * @since 1.0.0
...
public class b2JointIdTest {
     ...
     */
    private void struct() {
        try (b2JointId ptr = b2JointId.malloc()) {
     ...
     */
    private void buffer() {
        b2JointId.Buffer ptr = b2JointId.malloc(5);

        Assert.assertNotEquals(NULL, ptr.address());
        Assert.assertEquals(5, ptr.remaining());

        {
            try (b2JointId obj = b2JointId.malloc()) {
                obj.index1(1)
                   .world0((short) 10)
                   .generation((short) 2);
